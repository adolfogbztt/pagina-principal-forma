<!-- Row -->
<div class="content-row fadeout-element row_padding_top dark-section change-header-color" data-bgcolor="#000">
    <a href="/colegios/kinder" class="ajax-link" data-type="page-transition">
    <div class="one_half">
        <img src='/assets/colegios/kinder/k0.webp' class="item-image grayscale20" alt="forma producciones kinder"></img>
    </div>

    <div class="one_half last">
        <h1 class="has-mask-fill">Kinder</h1>
        <p class="has-animation" data-delay="50">El kinder es un tiempo de 
            descubrimiento, aprendizaje y diversión para los más pequeños. 
            En Forma, comprendemos la importancia de capturar los momentos 
            especiales y llenos de alegría de esta etapa tan especial en la 
            vida de los niños. A través de nuestros servicios de fotografía y 
            video, ayudamos a las familias y a las instituciones educativas a 
            preservar los recuerdos de los eventos en el kinder de una manera 
            auténtica y conmovedora.</p>

    </div>

    <hr class="destroy">
    </a>

</div>
<!--/Row -->


<!-- Row -->
{{-- <div class="content-row row_padding_bottom text-align-center full light-section " data-bgcolor="#fff">
    <div class="moving-gallery fw-gallery">
        <ul class="wrapper-gallery">
            <li>
                <img src='/assets/colegios/kinder/k1.webp' class="item-image grayscale20" alt="forma producciones kinder"></img>
            </li>
            <li>
                <img src='/assets/colegios/kinder/k2.webp' class="item-image grayscale20" alt="forma producciones kinder"></img>
            </li>
            <li>
                <img src='/assets/colegios/kinder/k3.webp' class="item-image grayscale20" alt="forma producciones kinder"></img>
            </li>
            <li>
                <img src='/assets/colegios/kinder/k4.webp' class="item-image grayscale20" alt="forma producciones kinder"></img>
            </li>
        </ul>
    </div>

    <div class="moving-gallery bw-gallery">
        <ul class="wrapper-gallery">
            <li>
                <img src='/assets/colegios/kinder/k5.webp' class="item-image grayscale20" alt="forma producciones kinder"></img>
            </li>
            <li>
                <img src='/assets/colegios/kinder/k6.webp' class="item-image grayscale20" alt="forma producciones kinder"></img>
            </li>
            <li>
                <img src='/assets/colegios/kinder/k4.webp' class="item-image grayscale20" alt="forma producciones kinder"></img>
            </li>
            <li>
                <img src='/assets/colegios/kinder/k5.webp' class="item-image grayscale20" alt="forma producciones kinder"></img>
            </li>
        </ul>
    </div>

</div> --}}
<!--/Row -->
