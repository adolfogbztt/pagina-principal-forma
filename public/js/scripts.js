      ...
      $("body").waitForImages({
        finished: function () {
          ...
          setTimeout(function () {
            $("body").waitForImages({
              finished: function () {
                gsap.to(
                  ...
                  { duration: 1, opacity: 1, delay: 0, ease: Power2.easeOut }
                );
              },
              waitForAll: true,
            });
            ...
            gsap.set(
              $("#showcase-carousel-holder .swiper-slide-active").prev().prev(),
              { x: -slideWidth, opacity: 0 }
            );
            gsap.set(
              $("#showcase-carousel-holder .swiper-slide-active").next().next(),
              { x: slideWidth, opacity: 0 }
            );
            gsap.set(
              $("#showcase-carousel-holder .swiper-slide-active").prev(),
              { x: -slideWidth, opacity: 0 }
            );
            gsap.set(
              $("#showcase-carousel-holder .swiper-slide-active").next(),
              { x: slideWidth, opacity: 0 }
            );
            gsap.to(
              $("#showcase-carousel-holder .swiper-slide-active").prev().prev(),
    ...
    if (!$("body").hasClass("disable-ajaxload")) {
      ...
      $(".next-ajax-link-project").on("click", function () {
        ...
        if ($(this).parents("#project-nav").hasClass("change-header")) {
          $("body").append(
            '<div class="temporary-hero"><div class="outer content-max-width"><div class="inner"></div></div></div>'
          );
        } else {
          $("body").append(
            '<div class="temporary-hero light-content"><div class="outer content-max-width"><div class="inner"></div></div></div>'
          );
        }
        ...
        $(".temporary-hero .next-hero-title span").each(function (
          index,
          element
        ) {
          TLNextHeroTitleSpan.to(
            element,
            {
              duration: 0.5,
              y: -140,
  ...
  function ShowcaseSlider() {
    if ($("#showcase-slider-holder").length > 0) {
        ...
        pagination: {
          ...
          renderBullet: function (index, className) {
            return (
              ...
              '<div class="slide-arrow translate-element">' +
      ...
      function BulletsPosition() {
        var bullets_count = $(".swiper-pagination-bullet .parallax-wrap")
          .length;
      ...
      if (!isMobile()) {
        ...
        $("#showcase-slider-holder .slide-title-wrapper").on(
          ...
          function () {
            ...
            $("#ball").append(
              '<p class="first">' +
                $this.find(".slide-title").data("firstline") +
                "</p>" +
                "<p>" +
                $this.find(".slide-title").data("secondline") +
                "</p>"
            );
...
---------------------------------------------------*/
  ...
  function ShowcaseCarousel() {
    if ($("#showcase-carousel-holder").length > 0) {
      ...
      if (!isMobile()) {
        $("#showcase-carousel-holder .slide-title-wrapper").on(
          "mousedown",
          function (event) {
            return false;
          }
    ...
    if ($(".showcase-list-holder").length > 0) {
      gsap.utils.toArray(".slide-list").forEach((section, index) => {
        ...
        const [x, xEnd] =
          index % 2
            ? [
                -((z.scrollWidth - section.offsetWidth) / 2) + 200,
                -((z.scrollWidth - section.offsetWidth) / 2) - 200,
              ]
            : [
                -((z.scrollWidth - section.offsetWidth) / 2) - 200,
...
---------------------------------------------------*/
  ...
  function StickyLists() {
    if ($(".showcase-reverse-list-holder").length > 0) {
      ...
      var leftImagesHeight =
        leftImages.length * imageHeight - window.innerHeight;

      var rightImages = gsap.utils.toArray(".aside.right .sr-slide");
      var rightImagesHeight =
        rightImages.length * imageHeight - window.innerHeight;
    ...
    if ($(".portfolio-wrap").length > 0) {
            ...
            finished: function () {
              gsap.utils
                ...
                .forEach((parallaxElement, index) => {
                  const parallaxElementChild = parallaxElement.querySelector(
                    ".item-parallax"
                  );
      ...
      if (!isMobile()) {
        ...
        $(".item-parallax").mouseenter(function (e) {
          ...
          $("#ball").append(
            '<p class="first">' +
              $this.parent().data("firstline") +
              "</p>" +
              "<p>" +
              $this.parent().data("secondline") +
              "</p>"
          );
          $this
            .closest(".item")
